<?php

namespace App\Repository;

use App\Entity\DemandeDecision;
use App\Entity\Enum\StatutDemande;
use App\Entity\Personnel;
use Doctrine\Bundle\DoctrineBundle\Repository\ServiceEntityRepository;
use Doctrine\Persistence\ManagerRegistry;

/**
 * @extends ServiceEntityRepository<DemandeDecision>
 */
class DemandeDecisionRepository extends ServiceEntityRepository
{
    public function __construct(ManagerRegistry $registry)
    {
        parent::__construct($registry, DemandeDecision::class);
    }

    /**
     * @return DemandeDecision[]
     */
    public function search(?Personnel $personnel, ?StatutDemande $statut): array
    {
        $qb = $this->createQueryBuilder('d')
            ->leftJoin('d.personnel', 'p')->addSelect('p')
            ->orderBy('d.createdAt', 'DESC');

        if ($personnel) {
            $qb->andWhere('d.personnel = :personnel')->setParameter('personnel', $personnel);
        }
        if ($statut) {
            $qb->andWhere('d.statut = :statut')->setParameter('statut', $statut);
        }

        return $qb->getQuery()->getResult();
    }
}
