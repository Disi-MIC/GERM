...
class MaterielInformatiqueImporter implements EntityImporterInterface
{
    ...
    public function getColumns(): array
    {
        return [
            ...
            new ImportColumnDefinition('date_mise_en_service', 'Date de mise en service (AAAA-MM-JJ)', false, '2026-03-01'),
            new ImportColumnDefinition('etat_code', "Code de l'état (obligatoire, voir Listes de valeurs)", true, 'en_stock'),
            new ImportColumnDefinition('service_code', 'Code du service de rattachement (dérivé automatiquement si affecte_a_matricule est renseigné, sinon facultatif)', false, 'DRH'),
    ...
    public function importRow(array $row, int $lineNumber): ImportRowResult
    {
        ...
        // Facultatif : dérivé du service de l'agent affecté si
        // affecte_a_matricule est renseigné (voir
        // MaterielInformatique::getService()/nettoyerServiceRedondant()) ;
        // sinon pas nécessaire du tout pour un matériel en stock/réformé.
        ...
        try {
            ...
            $dateMiseEnService = $this->parseDate($row['date_mise_en_service'] ?? null);
        ...
        $materiel->setDateMiseEnService($dateMiseEnService);
